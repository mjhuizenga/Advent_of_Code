...
borders = [[0]*len(arr[0]) for _ in range(len(arr))]
...
def calc_border(i, j):
    ...
    if i == 0 or arr[i-1][j] != ch:
        ...
        if j > 0 and arr[i][j-1] == ch and (i == 0 or arr[i-1][j-1] != ch):
            count += 0
        else:
            count += 1
    if j == 0 or arr[i][j-1] != ch:
        # print("Up:", j)
        if i > 0 and arr[i-1][j] == ch and (j == 0 or arr[i-1][j-1] != ch):
            count += 0
        else:
            count += 1
    if i == len(arr)-1 or arr[i+1][j] != ch:
        # print("Right:", i, arr[i+1][j])
        if j < len(arr[0])-1 and arr[i][j+1] == ch and (i == len(arr)-1 or arr[i+1][j+1] != ch):
            count += 0
        else:
            count += 1
    if j == len(arr[0])-1 or arr[i][j+1] != ch:
        # print("Down:", j, arr[i][j+1])
        if i < len(arr)-1 and arr[i+1][j] == ch and (j == len(arr[0])-1 or arr[i+1][j+1] != ch):
            count += 0
        else:
            count += 1
    return count
